...
class Cron extends CI_Controller
{
    ...
    {
        exit('Not allowed to visit these area.');
    }

    public function alv_fill_animals_label_periods()
    {
        $this->load->model('mdl_alv', '', true);
        $this->mdl_alv->fill_animals_label_periods();
    }
    ...
    public function KPI_data()
    {
        ...
        //check for value in the RED area of the threshold
        $data_1  = $data_default = array();
        foreach ($data as $key => $value) {
            $var_code = '';
            switch ($key) {
                case "expected_calving_interval":
                    $var_code = BBWEB_EXP_CALVING_INTERVAL;
                    break;
                case "average_days_first_service":
                    $var_code = BBWEB_AVERAGE_DAYS_TO_FS;
                    break;
                case "average_services_per_pregnancy":
                    $var_code = BBWEB_AVG_SERVICES_X_PREGN;
                    break;
                case "average_days_open":
                    $var_code = BBWEB_AVERAGE_DAYS_OPEN;
                    break;
                case "heat_detection_rate":
                    $var_code = BBWEB_HEAT_DETECTION_RATE;
                    break;
                case "conception_rate":
                    $var_code = BBWEB_CONCEPTION_RATE;
                    break;
                case "pregnancy_rate":
                    $var_code = BBWEB_PREGNANCY_RATE;
                    break;
                default:
                    break;
            }
            $data_default = unserialize($var_code);
            $s_v = 0;
            $e_v = 0;
            if (in_array("#FF0000", $data_default)) {
                $_key = array_search('#FF0000', $data_default);
                $rang_start = $_key-1;
                $rang_end = $_key+1;
                if (is_array($data_default[$rang_start])) {
                    $s_v = $data_default[$rang_start]['V'];
                } else {
                    $s_v = $data_default[$rang_start];
                }
                if (is_array($data_default[$rang_end])) {
                    $e_v = $data_default[$rang_end]['V'];
                } else {
                    $e_v = $data_default[$rang_end];
                }
            }
            if ($value['value'] >= $s_v && $value['value'] <= $e_v) {
                $data_1[] = array(
                                'role_id' => $role_id,
                                'category' => 'KPI',
                                'descrizione' => $key,
        ...
        $response = $this->mdl_dashboard->insert_KPI_service_message($data_1);
        if ($response) {
            echo "Success";
        } else {
